...
#ifdef STEPTHROUGH 
	std::memset(bdata, 0, sizeof(uint8_t));
#endif
}
CPU::~CPU() {
	delete rominfo;
	...
	while(isRunning) {
		if (!isPaused && 
			...
			if (Halted) {
...
#ifdef LOG_COMMANDS
				LogCommand(&oldPC, &inst, &param1, &param2, &params);
#endif
#ifdef STEPTHROUGH
				...
				if (skip > 0) {
				...
				else if (skipuntil > 0 && (skipuntil != registers->PC && skipuntil != (registers->PC - 1) && skipuntil != (registers->PC - 2))) {
					if (ShouldPrint()) {
						DisplayState();
					}
				}
					...
					if (x == 'q') { break; }
						...
						unsigned int input;
						scanf_s("%x", &input);
						printf("Memory[%04x] = %02x\n\n", (uint16_t)input, memory->get((uint16_t)input));
						goto debug_routine;
					}
					else if (x == 'd') {
						// Dump tile data to file
						int err = fopen_s(&fp, "FrameData.bin", "w");
						uint8_t val = memory->get(0x8000);
						fwrite(&val, sizeof(uint8_t), sizeof(uint8_t) * 0x1fff, fp);
						...
						for (int i = 0; i < BACKGROUNDTILES; i++) {
							for (int j = 0; j < 16; j++) {
								...
								bdata[(i * 16) + j] = draw->background[i]->data[j];
							}
						}
						...
						unsigned int input;
						scanf_s("%x", &input);
						printf("Cartridge[%05x] = %02x\n\n", (uint16_t)input, rominfo->GetCardridgeVal(input));
						goto debug_routine;
					}
					else if (x == 'i') {
						char buffer[11];
...
void CPU::Run(uint8_t opcode, uint8_t param1, uint8_t param2) {
	...
	case RETI:
		commands->RET_(opcode, &skipPCInc); InterruptsEnabled = 1; break;
	case RST_00H:
	case RST_10H:
	case RST_20H:
	case RST_30H:
...
void CPU::DisplayState() {
	...
	//	AF.a, AF.f, BC.b, BC.c, DE.d, DE.e, HL.h, HL.l, memory->getFlag(Z), memory->getFlag(N), memory->getFlag(H), memory->getFlag(C), SP, PC,
	//	memory->get(IF), memory->get(IE), memory->get(LY), memory->get(LYC), memory->get(STAT), memory->RomBank, memory->get(TAC), memory->get(TMA), memory->get(TIMA), memory->get(DIV), rDiv,
	//	IsLCDEnabled() ? "Yes" : "No");
...
void CPU::LogCommand(uint16_t* pc, uint8_t* inst, uint8_t* param1, uint8_t* param2, short* params) {
	...
	/*uint8_t spacer = 0xff;
	fwrite(&spacer, sizeof(uint8_t), sizeof(uint8_t), clog);*/
